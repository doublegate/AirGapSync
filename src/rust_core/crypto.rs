...
use ring::aead::{Aad, Nonce, NonceSequence, UnboundKey, SealingKey, OpeningKey, BoundKey};
...
pub fn encrypt(
    ...
    // Create unbound key
    let unbound_key = UnboundKey::new(algorithm, &key.key)
...
pub fn decrypt(
    ...
    let mut opening_key = OpeningKey::new(
        unbound_key,
        SingleUseNonce::new(nonce_bytes.to_vec())
    );
...
mod tests {
    ...
    fn test_key_derivation() {
        ...
        let key2 = CryptoKey::derive_from_password(
            password,
            salt,
            100_000,
            Algorithm::Aes256Gcm
