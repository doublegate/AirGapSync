...
fn test_crypto_integration() {
    ...
    let derived_key = CryptoKey::derive_from_password(
        password,
        &salt,
        100_000,
        Algorithm::Aes256Gcm,
...
fn test_asymmetric_keys() {
    ...
    let rsa_key = AsymmetricKey::generate(AsymmetricAlgorithm::Rsa2048)
        .expect("Failed to generate RSA key");
    
    let message = b"Test message for RSA signature";
    let signature = rsa_key.sign(message).expect("Failed to sign");
    rsa_key.verify(message, &signature).expect("Failed to verify");
    ...
    ecdsa_key.verify(message, &signature).expect("Failed to verify");
